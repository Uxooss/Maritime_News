...
async function scrapeCaribbean() {
  console.log("Scraping Caribbean MoU...");
  const url = 'https://caribbeanmou.org/';
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Caribbean MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'caribbean-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
      ...
      console.error(`Failed to fetch full text for Caribbean MoU article ${link.url}`, e.message);
    }
  }
...
module.exports = { scrapeCaribbean };
