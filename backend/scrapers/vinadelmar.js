...
async function scrapeVinaDelMar() {
  console.log("Scraping Viña del Mar...");
  const url = 'https://alvm.prefecturanaval.gob.ar/'; // their actual functional url is often this or alvm.prefecturanaval.gob.ar
  const backupUrl = 'https://alvm.prefecturanaval.gob.ar/';
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Viña del Mar index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'vina-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
        source: "Viña del Mar",
        category: "General",
        convention: "Latin American MoU",
        impact: "Medium",
      ...
      console.error(`Failed to fetch full text for Viña del Mar article ${link.url}`, e.message);
    }
  }
...
module.exports = { scrapeVinaDelMar };
