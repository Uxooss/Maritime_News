...
async function scrapeBlackSea() {
  console.log("Scraping Black Sea MoU...");
  const url = 'http://www.bsmou.org/';
  // Note: bsmou.org is sometimes http-only or has SSL issues, so we use http.
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Black Sea MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'blacksea-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
      ...
      console.error(`Failed to fetch full text for Black Sea MoU article ${link.url}`, e.message);
    }
  }
