...
async function scrapeMed() {
  console.log("Scraping Mediterranean MoU...");
  const url = 'https://www.medmou.org/';
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Med MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'med-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
      ...
      console.error(`Failed to fetch full text for Med MoU article ${link.url}`, e.message);
    }
  }
