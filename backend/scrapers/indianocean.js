...
async function scrapeIndianOcean() {
  console.log("Scraping Indian Ocean MoU...");
  const url = 'https://www.iomou.org/';
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Indian Ocean MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'indianocean-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
      ...
      console.error(`Failed to fetch full text for Indian Ocean MoU article ${link.url}`, e.message);
    }
  }
...
module.exports = { scrapeIndianOcean };
