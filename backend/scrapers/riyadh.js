...
async function scrapeRiyadh() {
  console.log("Scraping Riyadh MoU...");
  const url = 'https://www.riyadhmou.org/';
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Riyadh MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'riyadh-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
        source: "Riyadh MoU",
        category: "General",
        convention: "Riyadh MoU",
        impact: "Medium",
      ...
      console.error(`Failed to fetch full text for Riyadh MoU article ${link.url}`, e.message);
    }
  }
