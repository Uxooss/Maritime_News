...
async function scrapeParisMoU() {
  console.log("Scraping Paris MoU...");
  const url = 'https://www.parismou.org/';
  ...
  // Find article links
  const links = [];
  // Adjust selector to ParisMoU site (using generic approach for PoC since structure varies)
  $('a').each((i, el) => {
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Paris MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'parismou-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
        source: "Paris MoU",
      ...
      console.error(`Failed to fetch full text for Paris MoU article ${link.url}`, e.message);
    }
  }
