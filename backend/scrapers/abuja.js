...
async function scrapeAbuja() {
  console.log("Scraping Abuja MoU...");
  const url = 'https://abujamou.org/';
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Abuja MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'abuja-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
        source: "Abuja MoU",
        category: "General",
        convention: "Abuja MoU",
        impact: "Medium",
      ...
      console.error(`Failed to fetch full text for Abuja MoU article ${link.url}`, e.message);
    }
  }
