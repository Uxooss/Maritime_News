...
async function scrapeTokyo() {
  console.log("Scraping Tokyo MoU...");
  const url = 'https://www.tokyo-mou.org/';
  ...
  // Scrape index page for links.
  // For PoC on WordPress, we can look for generic article links
  $('a').each((i, el) => {
  ...
  if (links.length === 0) {
    throw new Error("No articles found on Tokyo MoU index page.");
  }
  ...
  for (const link of links) {
    try {
      ...
      const id = 'tokyo-' + crypto.createHash('md5').update(link.title).digest('hex').substring(0, 8);
      articles.push({
        id,
        title: link.title,
        source: "Tokyo MoU",
        category: "Safety",
        convention: "Tokyo MoU",
        impact: "Medium",
      ...
      console.error(`Failed to fetch full text for Tokyo MoU article ${link.url}`, e.message);
    }
  }
